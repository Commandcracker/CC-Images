...
local bbpack = require("bbpack")

local CCPal =
    io.lines and -- CC 1.55 adds this function, and also changes the colours, so...
    {
        [0] = {240, 240, 240},
        {242, 178, 51},
        {229, 127, 216},
...
local function simpleWindow(terminal, xPos, yPos, width, height)
    ...
    results.blit = function(_, _, colString)
        if curY < 1 or curY > height or curX > width or curX + #colString < 2 then
            return
        end
        ...
        if curX < 1 then
            ...
            curX = 1
        elseif ending ~= #colString then
            colString = colString:sub(1, ending)
        end
...
local function checkTerm(terminal)
    if not terminal.blit then
        terminal.blit = function(_, _, backCol)
            local counter, lastChar = 1, backCol:sub(1, 1)

            for i = 2, #backCol do
                if backCol:sub(i, i) ~= lastChar then
...
if term.setPaletteColour then
    function GIF.applyPalette(image, terminal)
...
function GIF.drawGIF(image, xPos, yPos, terminal)
    xPos, yPos, terminal = xPos or 1, yPos or 1, terminal or term
    if type(image[1][1]) == "table" then
        image = image[1]
    end
    checkTerm(terminal)
...
function GIF.animateGIF(image, xPos, yPos, terminal)
    if terminal == term or not terminal then
    ...
    while true do
        for i = 1, #image do
            ...
            if i == 1 then
                buffer.clear(image.backgroundCol2)
            end

            GIF.drawGIF(imageI, 1, 1, buffer)

            if i == 1 then
                buffer.draw()
            elseif imageIneg1.disposal == 2 then
                buffer.draw(
                    math.min(imageIneg1.xstart, imageI.xstart) + 1,
                    math.min(imageIneg1.ystart, imageI.ystart) + 1,
                    math.max(imageIneg1.xstart + imageIneg1.xend, imageI.xstart + imageI.xend),
                    math.max(imageIneg1.ystart + imageIneg1.yend, imageI.ystart + imageI.yend)
                )
            else
                buffer.draw(
                    imageI.xstart + 1,
                    imageI.ystart + 1,
                    imageI.xstart + imageI.xend,
                    imageI.ystart + imageI.yend
                )
            end
...
local function rasteriseGIF(image)
    local results, buffer = {
            ["width"] = image.width,
            ["height"] = image.height,
            ["backgroundCol"] = image.backgroundCol,
            ["backgroundCol2"] = image.backgroundCol2,
            ["pal"] = image.pal
        },
        {}
        ...
        for y = 1, thisImg.yend do
            ...
            for i = 1, #thisImgY do
                ...
                elseif type(thisImgYI) == "string" then
                    for j = 1, #thisImgYI do
                        buffY[x + j - 1] = colourNum[thisImgYI:sub(j, j)]
                    end
                    x = x + #thisImgYI
                else
                    for j = 1, #thisImgYI do
                        buffY[x + j - 1] = thisImgYI[j]
                    end
                    x = x + #thisImgYI
                end
            end
...
function GIF.flattenGIF(image, optimise, prerastered)
    if not prerastered then
    ...
    for i = 1, #image do
        ...
        if cacheImg then
            cxStart, cxEnd, cyStart, cyEnd = cacheImg.xstart, cacheImg.xend, cacheImg.ystart, cacheImg.yend
        end
            ...
            for y = 1, #thisImg do
                ...
                for x = 1, width do
                    if
                        thisImgY[x] ~=
                            ((prevDisp == 2 and
                                not (x < cxStart + 1 or y < cyStart + 1 or x > cxStart + cxEnd or y > cyStart + cyEnd)) and
                                " " or
                                cacheImgY[x])
                     then
                        same = false
        ...
        if same and i > 1 then
            ...
            if i - bump ~= #image and optimise ~= nil then
                ...
                for y = 1, #thisImg do
                    ...
                    for x = 1, width do
                        if thisImgY[x] ~= " " and nextImgY[x] == " " then
                            obscured = false
                            if y < tyStart + 1 then
                                tyStart = y - 1
                            end
            ...
            if txStart > txEnd then
                txStart, txEnd, tyStart, tyEnd = 0, 1, 0, 1
            end

            local nxStart, nxEnd, nyStart, nyEnd, cacheDisp =
                txStart,
                txEnd - txStart,
                tyStart,
                tyEnd - tyStart,
                cacheImg and cacheImg.disposal
            newImg = {
                ["delay"] = thisImg.delay,
                ["disposal"] = thisImg.disposal,
                ["xstart"] = nxStart,
                ["xend"] = nxEnd,
                ["ystart"] = nyStart,
                ["yend"] = nyEnd
            ...
            for y = 1, nyEnd do
                local skip, chars, temp, thisImgY, cacheImgY =
                    0,
                    {},
                    {},
                    thisImg[y + nyStart],
                    cacheImg and cacheImg[y + nyStart]
                for x = nxStart + 1, nxStart + nxEnd do
                    local thisVal = thisImgY[x]
                    if
                        optimise ~= nil and cacheImg and thisVal == cacheImgY[x] and
                            (cacheDisp == 1 or
                                (cacheDisp == 2 and
                                    (x < cxStart + 1 or y + nyStart < cyStart + 1 or x > cxStart + cxEnd or
                                        y + nyStart > cyStart + cyEnd)))
                     then
                        thisVal = " "
                    end
...
function GIF.resizeGIF(image, width, height)
    if not width then
        width = math.floor(height / image.height * image.width)
    end
    if not height then
    ...
    for i = 1, #image do
        ...
        for y = 1, height do
            ...
            for x = 1, width do
                ...
                while remY > 0 do
                    local remX, maxInc, oldFrameY, xPos, yWeight =
                        xInc,
                        (math.floor(yPos) == yPos) and 1 or (math.ceil(yPos) - yPos),
                        oldFrame[math.min(math.floor(yPos), image.height)],
                        xPos2
                    if remY < maxInc then
                        yWeight = remY
                        remY = 0
                    ...
                    while remX > 0 do
                        local maxInc, thisChar, xWeight =
                            (math.floor(xPos) == xPos) and 1 or (math.ceil(xPos) - xPos),
                            oldFrameY[math.min(math.floor(xPos), image.width)]
                        if remX < maxInc then
                            xWeight = remX
                            remX = 0
                        else
                        ...
                        if thisChar ~= " " then
                            ...
                            R, G, B =
                                R + thisChar[1] * thisWeight,
                                G + thisChar[2] * thisWeight,
                                B + thisChar[3] * thisWeight
                        end
    ...
    return GIF.flattenGIF(image, false, true)
end

function GIF.loadGIF(targetfile, pal)
    local results, fileread = {["backgroundCol"] = 0}, fs.open(targetfile, "rb")
    if not fileread then
        error("GIF.loadGIF(): Unable to open " .. targetfile .. " for input.", 2)
    end
    ...
    if type(pal) == "string" then
        ...
        if not pal[0] then
            pal[0] = {1000, 1000, 1000, "minecraft:air", 0}
        end
        results.pal = pal
    elseif pal then
        -- Construct a reduced palette from the GIF's own.
        if not fileread.readAll then
            error("Installed version of CC does not support palette alterations.")
        end
        ...
        if bit.band(temp, 128) == 128 then
            for i = 1, 2 ^ (bit.band(temp, 7) + 1) do
                local thisCol, found = fileread.read(3), false

                for i = 1, counter do
                    if pal[i] == thisCol then
                        found = true
                        break
                    end
                end
        ...
        while true do
            ...
            elseif record == 44 then
                ...
                if bit.band(record, 128) == 128 then
                    for i = 1, 2 ^ (bit.band(record, 7) + 1) do
                        local thisCol, found = fileread.read(3), false
        ...
        while #pal > 16 do
            ...
            for i = 1, curLen - 1 do
                ...
                for j = i + 1, curLen do
                    local palJ = pal[j]
                    local thisDiff =
                        math.abs(palJ[1] - palI[1]) + math.abs(palJ[2] - palI[2]) + math.abs(palJ[3] - palI[3])
            ...
            for i, j in pairs(matches) do
                ...
                pal[i] = {
                    math.floor((palI[1] + palJ[1]) / 2),
                    math.floor((palI[2] + palJ[2]) / 2),
                    math.floor((palI[3] + palJ[3]) / 2)
                }
    ...
    local readBytes = fileread.readAll and function(num)
            local output = readByte(num)
            return {output:byte(1, #output)}
        end or function(num)
            local output = {}
            for i = 1, num do
                output[#output + 1] = readByte()
            end
            return output
        end
    ...
    for i = 1, 6 do
        if temp[i] ~= GIF87a[i] and temp[i] ~= GIF89a[i] then
            fileread.close()
    ...
    if globalPal then
        globalPal = {}
        for i = 0, globalColourCount - 1 do
            globalPal[i] = readBytes(3)
        end
    end
    ...
    while true do
        ...
        while true do
            ...
            if record == 33 then
                ...
                elseif record == 254 then
                    ...
                    repeat
                        ...
                        if length > 0 then
                            fullComment[#fullComment + 1] = string.char(unpack(readBytes(length)))
                        end
                    until length == 0
                ...
                elseif record == 1 then
                    ...
                    repeat
                        local length = readByte()
                        if length > 0 then
                            fullText[#fullText + 1] = string.char(unpack(readBytes(length)))
                        end
                    until length == 0
            ...
            elseif record == 44 then
                ...
                if bit.band(record, 128) == 128 then
                    ...
                    for i = 2, 2 ^ (bit.band(record, 7) + 1) do
                        thisPal[#thisPal + 1] = readBytes(3)
                    end
                else
                    -- Global colour map:
                ...
                for i = 0, #thisPal do
                    ...
                    for j = 0, #pal do
                        local palJ = pal[j]
                        local thisDiff =
                            math.abs(palJ[1] - thisPalI[1]) + math.abs(palJ[2] - thisPalI[2]) +
                            math.abs(palJ[3] - thisPalI[3])
            ...
            elseif record == 59 then
                -- End of file.
                fileread.close()
                return GIF.flattenGIF(results, false, true)
            end
        end
        ...
        if thisImg.disposal == 3 then
            ...
            for y = 1, math.min(height - yStart, yEnd) do
                ...
                for x = 1, math.min(width - xStart, xEnd) do
                    cacheBackupY[x] = cacheFrameY[x + xStart]
                end
        ...
        while y < yEnd + 1 do
            ...
            if y <= height then
                for x = 1, xEnd do
                    ...
                    if thisVal ~= transparent and x + xStart <= width then
                        cacheFrameY[x + xStart] = thisPal[thisVal]
                    end
                end
            else
                for x = 1, xEnd do
        ...
        for y = 1, height do
            ...
            for x = 1, width do
                thisImgY[x] = cacheFrameY[x]
            end
            thisImg[y] = thisImgY
        end
        if not thisImg.delay then
            thisImg.delay = 0.1
        end
        ...
        elseif thisImg.disposal == 3 then
            for y = 1, math.min(height - yStart, yEnd) do
                ...
                for x = 1, math.min(width - xStart, xEnd) do
                    cacheFrameY[x + xStart] = cacheBackupY[x]
                end
            end
            cacheBackup = nil
        end
...
function GIF.setBackgroundColour(image, colour)
    if type(colour) ~= "number" and not colourNum[colour] then
        error("GIF.setBackgroundColour: Bad colour.", 2)
    end
...
GIF.setBackgroundColor = GIF.setBackgroundColour

function GIF.toPaintutils(image)
    if type(image[1][1]) == "table" then
        image = image[1]
    end
    ...
    for y = 1, image.yend do
        ...
        for i = 1, #imageY do
            ...
            if type(imageYI) == "number" then
                for j = 1, imageYI do
                    resultsY[#resultsY + 1] = 0
                end
            else
                for j = 1, #imageYI do
                    resultsY[#resultsY + 1] = 2 ^ colourNum[imageYI:sub(j, j)]
                end
            end
        end
        results[y] = resultsY
    ...
    for i = #results[1] + 1, image.xstart + image.xend do
        results[1][i] = 0
    end
...
function GIF.buildGIF(...)
    ...
    return GIF.flattenGIF(arg, true, true)
end

function GIF.saveGIF(image, file)
    if not (image.width or image.xstart) then
        -- "paintutils" image.
        image = GIF.buildGIF(image)
    ...
    if not file then
        error("Unable to open file for output.", 2)
    end
    ...
    if image.pal then
        ...
        for i = #pal + 2, 2 ^ power do
            writeBytes({127, 127, 127})
        end
    else
        writeByte(132)
        transCol = #CCPal + 1
    ...
    if #image > 1 then
        ...
        for i = 1, 11 do
            writeByte(string.byte("NETSCAPE2.0", i))
        end
        ...
        for y = 1, image.yend do
            ...
            for i = 1, #imageY do
                ...
                elseif type(imageYI) == "string" then
                    for j = 1, #imageYI do
                        writeByte(colourNum[imageYI:sub(j, j)])
                    end
                    rowLen = rowLen + #imageYI
                else
                    for j = 1, #imageYI do
